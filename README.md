...
the `gp credential-helper` generate or fetch a token via the supervisor's api and a proxy (outside of pod) translates that token into the git provider token (filtered by git agent)
